...
export function normalizeRateLimits(raw: Record<string, unknown>): RateLimitSnapshot {
  ...
  return {
    ...
    sonnet: sonnet
      ? {
          usedPercent: asNumber(sonnet.usedPercent ?? sonnet.used_percent),
          windowDurationMins: (() => {
            const value = sonnet.windowDurationMins ?? sonnet.window_duration_mins;
            if (typeof value === "number") {
              return value;
            }
            if (typeof value === "string") {
          ...
          resetsAt: (() => {
            const value = sonnet.resetsAt ?? sonnet.resets_at;
            if (typeof value === "number") {
              return value;
            }
            if (typeof value === "string") {
