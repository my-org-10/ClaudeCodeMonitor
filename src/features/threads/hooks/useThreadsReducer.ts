...
export function threadReducer(state: ThreadState, action: ThreadAction): ThreadState {
  switch (action.type) {
    ...
    case "upsertItem": {
      ...
      if (isUserMessage && action.workspaceId) {
        const renameText = formatThreadName(item.text) ?? "";
        const hadUserMessage = list.some(
        ...
        const updatedThreads = threads.map((thread) => {
          ...
          const shouldRename =
            ...
            looksAutoGeneratedThreadName(thread.name) &&
            !action.hasCustomName;
          return shouldRename ? { ...thread, name: renameText } : thread;
        });
