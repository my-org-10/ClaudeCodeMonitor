...
export type ThreadAction =
  ...
  // Optimistic user message - pairs with deduplication in threadItems.ts
  | {
      type: "addUserMessage";
      workspaceId: string;
      threadId: string;
...
export function threadReducer(state: ThreadState, action: ThreadAction): ThreadState {
  switch (action.type) {
    ...
    // Creates local message (ID: timestamp-user), replaced by server version via deduplication
    case "addUserMessage": {
      ...
      const renameText = extractRenameText(textValue);
      ...
      const updatedThreads = threads.map((thread) => {
        ...
        const nextName =
          shouldRename && renameText.length > 38
            ? `${renameText.slice(0, 38)}…`
            : shouldRename
              ? renameText
              : thread.name;
    ...
    case "upsertItem": {
      ...
      const updatedItems = prepareThreadItems(
